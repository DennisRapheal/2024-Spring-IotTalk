...
const dai = function (profile, ida=undefined) {
    var exec_interval=500;
    var odf_func = {};
    var idf_func = {};
    //var mqtturl = 'ws://' + window.location.hostname + ':8866/mqtt';        //unencrypted connections
    var mqtturl = 'wss://' + window.location.hostname + ':8866/mqtts';    //encrypted connections	
    var user = 'iottalk';
    var password = 'iottalk2023';
    ...
    dan.init(push, pull, csmapi.get_endpoint(), mac_addr, profile, init_callback, mqtturl, exec_interval);

    function switch_to_http(){
        mqtt_client.end();
        mqtturl = undefined;
        profile['mqtt_enable'] = false;
        dan.init(push, pull, csmapi.get_endpoint(), mac_addr, profile, init_callback, undefined, exec_interval);
    }
